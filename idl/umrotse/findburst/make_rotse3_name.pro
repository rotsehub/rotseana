function make_rotse3_name, rain, decin, tele=tele, space=space
...
if n_params() eq 0 then begin
  print,'syntax-make_rotse_name,rain,decin,tele=tele,space=space'
  return,-1
endif
ra=sixty(rain/15.0)
dec=sixty(decin)
...
if keyword_set(space) then sepchar = ' ' else sepchar = '_'
ns = startstr+sepchar+'J'+ras(0)+ras(1)+ras(2)+sign+decs(0)+decs(1)+decs(2)
