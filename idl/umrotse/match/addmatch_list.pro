pro addmatch_list, obj_template, file, keep=keep, skip=skip, $
	iter=iter, subr=subr, fail=fail, ebox=ebox
;+
; NAME:	ADDMATCH_LIST
;
; CALLING SEQUENCE:	addmatch_list, obj_template, file
;
...
;	Tim McKay		UM	Alter from addmatch_s to handle an 
				;	an input list of files.
;	Tim Mckay		UM	Add ebox argument
;******************************************************************************

 if N_params() eq 0 then begin
        print,'Syntax - addmatch_list, match, file, keep=keep, skip=skip, iter=iter, subr=subr,fail=fail, ebox=ebox'
        return
 endif
...
;Now close match the two
    ...
    nbad=where(abs(obj_template.rmag(m1)-mnew) gt sigma,nfound) 
    info=size(nbad)
