pro new_coadd,match,refobs,obslist,imtot,threshold=threshold
;+
; NAME:	NEW_COADD
;
; CALLING SEQUENCE: new_coadd,match,obslist,imtot,threshold=threshold
;
...
;
 ...
 if N_params() eq 0 then begin
        print,'Syntax - new_coadd,match,refobs,obslist,imtot,threshold=threshold'
        return
 endif
...
;Now project the ras and decs of these to the plane:
	...
	n=n_elements(j)
	num=fix(0.1*n)
	imw=warp_tri(xr(j(0:num)),yr(j(0:num)),$
		xo(j(0:num)),$
		yo(j(0:num)),im)
