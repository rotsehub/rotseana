pro tycho_recalib, match, grid=grid
;+
; NAME:	tycho_recalib
;
; CALLING SEQUENCE:	tycho_recalib, match, newmatch
;
; INPUTS:	match: match structure produced by catmatch_s or addmatch_s
;
; OUTPUTS:	newmatch: copy of match calibrated relatively
;	
; INPUT KEYWORDS:
;			
...
;
; REVISION HISTORY:  
;	Tim McKay		UM		11/10/98	
;******************************************************************************

 if N_params() eq 0 then begin
        print,'Syntax - tycho_recalib, match, grid=grid'
        return
 endif
...
; newmatch.m(0,f)=mnew(f)
 ...
 for i=1,nimages,1 do begin
	...
	relphoto_map,match,0,i,nmags,nbox=grid,frac=0.2
	j=where(nmags(1,*) gt -1 and nmags(1,*) lt 30)
	match.m(i,j)=nmags(1,ij)
