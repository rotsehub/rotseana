pro ss_detect_and_plot,newsubs,coadddir=coadddir,respdir=respdir,thumbfile=thumbfile,catalog=catalog,varlist=varlist,cand_list=cand_list,reflist=reflist,refdir=refdir,oldcanddir=oldcanddir,subrefsky=subrefsky
...
;translate candidate position into ra and dec.
    ...
    cat=mrdfits(catalog,1,/silent)
    nfind=0
    nrelax=0
    relax=bytarr(n_elements(sobj0))
    close_match_radec,sobj0.ra,sobj0.dec,cat.ra,cat.dec,m11,m12,pixscale*3.,1,miss1,/silent
    if m11[0] ne -1 then relax[m11]=1
...
;test the fwhm
    ...
    fwhm2=fwhm2>2.5
    fwhmr=fwhmr>2.5
    if nconv eq 1 and crefstat.fwhm gt 1.5 then fwhmlim0=(crefstat.fwhm*2) >7.5 else fwhmlim0=((fwhm0+fwhmr)*2.0)>7.5
    fwhmlim1=((fwhm1-fwhm0)*2.+fwhmlim0) >7.5 ;((fwhm1+fwhmr)*1.5)>7.5
    fwhmlim2=((fwhm2-fwhm0)*2.+fwhmlim0) >7.5;((fwhm2+fwhmr)*1.5)>7.5
...
;catalog match
    ...
    for i=0,nfind-1 do begin
        mcase=3
        gcirc,1,sobj0[i].ra/15d0,sobj0[i].dec,cat.ra/15d0,cat.dec,dist
        catdist=dist/(cat.major+secpixscale>2*secpixscale)
        ...
        case mcase of
            1:if percent ge 0.08 then fluxtst[i]=1b
            2:if percent ge 0.60 then fluxtst[i]=1b
            3:if percent ge 0.12 then fluxtst[i]=1b
            0:if percent ge 0.03 then fluxtst[i]=1b 
        endcase

        if (percent ge 8. ) and fluxtst[i] eq 1b then begin
...
;need further filtering
        ...
        if nlow gt 20 then begin
            rad=[800,600,400]
            i=0
            while nlow gt 20 and i lt n_elements(rad) do begin
                sobj=sobj[tst]
                xl=w_image/2-rad[i]
                xh=w_image/2+rad[i]
                yl=h_image/2-rad[i]
    ...
    if nlow+nhigh gt 30 and nhigh gt 20 then begin
        sobj=sobj0[highp]
        nbtst=bytarr(nhigh)
        ...
        if nlow+nhigh gt 30 and nhigh gt 20 then begin
            rad=[800,600,400]
            i=0
            while nhigh gt 20 and i lt n_elements(rad) do begin
                sobj=sobj[tst]
                xl=w_image/2-rad[i]
                xh=w_image/2+rad[i]
                yl=h_image/2-rad[i]
...
if nfind gt 0 then begin
    ...
    jpegnamebase=jpegnames[0]+'_ss_'+jpegnames[1]+'_'+strmid(jpegnames[2],0,2)
