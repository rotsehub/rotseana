...
; in a structure.      Removed this rubbish  Erin Sheldon
...
;if not mrd_substruct(table, column) then begin   ;Erin Sheldon
   table.(column) = value
;endif else begin
...
if strlen(name) gt 15 then name = strmid(name, 0, 15) 
w = where(name eq strmid(namelist, 0, 15) )
...
;=====================================================================
; END OF GeneralUTILITY FUNCTIONS ===========================================
;=====================================================================
...
; Struct_version Type of structures used
;
...
pro mrd_read_image, unit, range, rsize, table 
...
pro mrd_image, header, range, rsize, table, scales, offsets, scaling, $
...
skip_dist = fxpar(header, 'NAXIS1')
skip_dist = skip_dist*(fxpar(header, 'NAXIS2') - (range[1]+1))



mrd_skip, unit, skip_dist + heapoff-sz 
...
pro mrd_read_table, unit, range, rsize, structyp, nfld, typarr, table, $
                                      old_struct=old_struct
...
pro mrd_table, header, structyp, use_colnum,           $ 
    range, rsize, table, nfld, typarr, fnames, fvalues,   $ 
...
function mrdfits3, unit, extension, oextension, header,  $
	structyp = structyp,                    $
	range = range,                          $
...
common mrdfits3_block,use_colnum  , rsize, tab, nfld, typarr,        $ 
	 fnames, fvalues, vcls, vtpes, scales, offsets, scaling, status, $
	 tempdir, columns, no_tdim
...
; Check positional arguments.
   ...
   if n_params() le 0  or n_params() gt 4 then begin
   	print,'MRDFITS3: Usage'
	print,'   a=mrdfits3(unit, [extension, oextension, header],'
        print,'              structyp = structyp,'
	print,'              range = range, '
	print,'              dscale = dscale, fscale=fscale,'
	print,'              silent = silent,'
	print,'              error_action = error_action,'
...
junk = fxposit2(unit, extension,oextension)

if junk lt 0 then begin
  	print, 'MRDFITS: File access error',junk
	return, 0
endif
...
num_objects=fxpar(header,'NAXIS2')
arange=[0,num_objects-1]

;dave johnston addition
...
if extension gt 0 then begin
	xten = strtrim( fxpar(header,'XTENSION'),2)
	if xten eq 'IMAGE' then type = 0   $
...
if type eq 0 then begin
	...
	mrd_image, header, arange, rsize, table, scales, offsets, status, $
	  silent=silent, tempdir=tempdir
	if status ge 0 and rsize gt 0 then mrd_read_image, unit, arange, rsize, table
	size = rsize
...
if not keyword_set(deja_vu)  then begin
	 ...
	 arange, rsize, table, nfld, typarr,        $ 
	 fnames, fvalues, vcls, vtpes, scales, offsets, scaling, status, $
	 silent=silent, tempdir=tempdir, columns=columns, $
	 no_tdim=no_tdim, old_struct=old_struct
	
	 tab=table(0)
	
endif else BEGIN
	table=replicate(tab,num_objects)
ENDELSE
     ...
     if status ge 0  and  size gt 0  then begin
        ...
        ;*** Read data.
	point_lun,-1*unit,pos1
     	mrd_read_table, unit, arange, rsize,  $
	  structyp, nfld, typarr, table, old_struct=old_struct
	point_lun,-1*unit,pos2
	dd=fix((pos2-pos1+2879)/2880L)
	dd=dd*2880L-pos2+pos1
;	print,pos2-pos1,dd
	mrd_skip, unit, dd
     	if status ge 0 and keyword_set(columns) then $
