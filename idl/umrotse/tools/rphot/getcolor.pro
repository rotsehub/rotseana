...
IF np EQ 0 THEN BEGIN
   ...
   IF Keyword_Set(trueColor) THEN BEGIN
      returnColor = LonArr(n_elements(names))
      FOR j=0,n_elements(names)-1 DO returnColor[j] = Color24([rvalue[j], gvalue[j], bvalue[j]])

         ; If LOAD keyword set, return a color structure.

      IF Keyword_Set(load) THEN BEGIN
         returnValue = Create_Struct('black', returnColor[0])
         FOR j=1,n_elements(names)-1 DO returnValue = Create_Struct(returnValue, names[j], returnColor[j])
   ...
   IF Float(!Version.Release) GE 5.2 THEN BEGIN
      ...
      IF Keyword_Set(load) THEN BEGIN
         TVLCT, Reform([rvalue, gvalue, bvalue], n_elements(names), 3), start
         returnValue = Create_Struct('black', start)
         FOR j=1,n_elements(names)-1 DO returnValue = Create_Struct(returnValue, names[j], start+j)
         RETURN, returnValue
      ENDIF

      returnColor = REFORM([rvalue, gvalue, bvalue], n_elements(names), 3)
      RETURN, returnColor
