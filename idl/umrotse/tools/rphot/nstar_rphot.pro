pro nstar_rphot,image,id,xc,yc,mags,sky,group,phpadu,readns,psfname,DEBUG=debug, $  
...
;-
 ...
 ;; do we need to read in the psf?  --RQ 050512
...
RESTART: 
 ...
 if reset EQ 1b then begin
     if not keyword_set(norecentroid) then begin
         xb = xg + ixmin
         yb = yg + iymin
     endif
 endif
...
REM_FAINT: 
 if keyword_set(keepfaint) eq 0 and (faint GE 0.25) or (nfaint GT 0) then begin ;; added keepfaint  -RQ 050510
         if not SILENT then $
