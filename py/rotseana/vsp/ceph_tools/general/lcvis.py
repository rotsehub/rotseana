...
def run_chisqbintest(event):
    ...
    if clicked[0] == 'Run':
        ...
        try: 
            results = subprocess.run([sys.executable, os.path.dirname(__file__) + "/csbt.py", f"{str(chibin_data)}", "-n", f"{numpoints}", "-w", f"{window}", "-m", f"{method}", "-c", "lcs", "-p", "false"], stdout=subprocess.PIPE)
