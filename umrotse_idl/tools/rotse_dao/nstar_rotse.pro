pro nstar_rotse,image,id,xc,yc,mags,sky,group,phpadu,readns,psfname,DEBUG=debug, $  
          errmag,iter,chisq,peak,PRINT=print,SILENT=silent, VARSKY = varsky, $
          keepfaint=keepfaint,flux=flux,weightim=weightim
;+
; NAME:
;       NSTAR
; PURPOSE:
...
RESTART: 
 ...
 if (n_elements(weightim) gt 0) then begin
     wimage=weightim[ixmin:ixmax,iymin:iymax]
 endif
...
;                                        Offset to the subarray
 ...
 if (n_elements(weightim) gt 0) then iwimage=wimage[igood]
 dpos = (idimage-d) > 0
 sigsq = dpos/phpadu + ronois + (0.0075*dpos)^2 + (pkerr*(dpos-skybar))^2
 if (n_elements(weightim) gt 0) then begin
     sigsq=sigsq + 1/iwimage
 endif
...
REM_FAINT: 
 if (not keyword_set(keepfaint)) and (faint GE 0.25) or (nfaint GT 0) then begin
         if not SILENT then $
