...
;*      ROW_CHECK.PRO                                                         *
;*                                                                            *
;*      ROW_CHECK tests the row alignment of CCD images by comparing lists of *
;*  "hot pixels" taken from successive image frames. The alignment most       *
;*  common to all images defines the zero shift reference. This program       *
;*  operates on all primary image files located in the directory specified by *
;*  the environmental variable, STAR_LIST_DIR. Each of the four ROTSE-I       *
;*  cameras is processed separately. Dark frames with zero exposure length    *
;*  are ignored since they cannot be correlated. A summary of the results are *
;*  recorded in the file, row_check.lst.                                      *
;*                                                                            *
;*                                                                            *
;*      Carl W. Akerlof                                                       *
...
;******************************************************************************
      ...
      T_BEGIN=SYSTIME(1)
      GET_LUN, P_UNIT
      OPENW, P_UNIT, 'row_check.lst'
      PRINTF, P_UNIT, "ROW_CHECK image file list", FORMAT='(/A/)'
      GET_LUN, UNIT
      FOR I_CAM = 0, 3 DO BEGIN
               ...
               IF (STRPOS(FILE_NAME, 'drk0000', 0) LT 0) THEN BEGIN
                  ...
                  IF ((D(0) GE 48) AND (D(0) LE 57)) THEN BEGIN
                     ...
                     IF (FS.SIZE GT 0) THEN BEGIN
                        ...
                        I=I+1
                     ENDIF ELSE BEGIN
                        PRINT, 'Null file: ', FILE_NAME
                        PRINTF, P_UNIT, 'Null file: ', FILE_NAME
                     ENDELSE
                     CLOSE, UNIT
                  ENDIF
               ENDIF
         ...
         FOR I_FILE = 1, N_FILES-1 DO BEGIN
            ...
            INDX=WHERE(HIT_CNT GT 1, N_MULTI)
            IF (N_MULTI GT 0) THEN BEGIN
               XV=INDX MOD L_COL
               YV=(INDX/L_COL)+N_DIF
               PRINT, FILES(I_FILE-1), FORMAT='("Multi-hit: ",A)'
               PRINT, FILES(I_FILE), FORMAT='("           ",A)'
               PRINTF, P_UNIT, FILES(I_FILE-1), FORMAT='("Multi-hit: ",A)'
               PRINTF, P_UNIT, FILES(I_FILE), FORMAT='("           ",A)'
               I=0
               J=0
               S='           '
               WHILE (I LT N_MULTI) DO BEGIN
                  SX=STRING(XV(I), FORMAT='(I4)')
                  SY=STRING(YV(I), FORMAT='(I4)')
                  S=S+'['+SX+','+SY+']'
                  I=I+1
                  J=J+1
                  IF (J GE 6) THEN BEGIN
                     PRINT, S
                     PRINTF, P_UNIT, S
                     S='           '
                     J=0
         ...
         IF (N_FILES GT 0) THEN BEGIN
            ...
            FOR I_FILE = 0, N_FILES-1 DO BEGIN
               S=''
               FOR I = 0, 1 DO BEGIN
                  IF (MATCH(I,I_FILE) GT 0) THEN BEGIN
                     IF (I EQ 0) THEN S=' ' ELSE S=S+' ['
                     S=S+STRING(N_OFF(I,I_FILE), FORMAT='(I2)')
                     S=S+STRING(MATCH(I,I_FILE), FORMAT='(I4)')
                     S=S+STRING(ROW_LIM(I,I_FILE), FORMAT='(I5)')
                     S=S+STRING(ROW_LIM(I+2,I_FILE), FORMAT='(I5)')
                     IF (I EQ 1) THEN S=S+'] '
                  ENDIF ELSE BEGIN
                     S=S+S_DEF(I)
                  ENDELSE
               ENDFOR
               I=RSTRPOS(FILES(I_FILE), '/')
               IF (I GT 0) THEN BEGIN
                  F_NAME=STRMID(FILES(I_FILE), I+1, STRLEN(FILES(I_FILE))-I-1)
               ENDIF ELSE BEGIN
                  F_NAME=FILES(I_FILE)
               ENDELSE
               PRINT, S, F_NAME, FORMAT='(A,A)'
               PRINTF, P_UNIT, S, F_NAME, FORMAT='(A,A)'
            ENDFOR
         ENDIF
      ENDFOR
      FREE_LUN, UNIT
