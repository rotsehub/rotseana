pro aper_rphot,image,xc,yc,mags,errap,sky,skyerr,phpadu,apr,skyrad,badpix, $
...
;        negative number
;
   if chk_badpix then begin
     minthisapd = min(thisapd, max = maxthisapd)
     if ( (minthisapd LE badpix[0] ) or ( maxthisapd GE badpix[1]) ) then $
     apmag[k] = -555.55 else apmag[k] = total(thisapd*fractn) 
   endif else $
 ...
 ;; ##### Quimby hack to allow negative flux return #####
 ngood=n_elements(apmag)
 good=lindgen(ngood)
 ...
 nbad=0                         ; ##### Quimby hack to allow negative flux return #####
